...
{
//	session_start();
//$captcha_code = $_POST['c'];
	$image = imagecreatetruecolor(100, 40);    //1>设置验证码图片大小的函数
//5>设置验证码颜色 imagecolorallocate(int im, int red, int green, int blue);
//	$bgcolor = imagecolorallocate($image, rand(0,255), rand(0,255), rand(0,255)); //#ffffff
//    $bgcolor = imagecolorallocate($image, 0, 0, 0);
    $white = imagecolorallocate ( $image, 255, 255, 255 );
//6>区域填充 int imagefill(int im, int x, int y, int col) (x,y) 所在的区域着色,col 表示欲涂上的颜色
	imagefill($image, 100, 28, $white);
//10>设置变量
	$captcha_code = "";
//7>生成随机的字母和数字
	for ($i = 0; $i < 4; $i++) {
